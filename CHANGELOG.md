...
- Simple and advanced map generator using the installed Factorio version's
  native `map-gen-settings` and `map-settings` files.
- Native and reusable custom map presets with seed support.
- Map previews generated by the installed Factorio binary before world creation.
